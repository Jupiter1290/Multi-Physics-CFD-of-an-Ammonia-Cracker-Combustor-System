 ...
 * CONFIGURATION
 *--------------------------------------------------------------------------*/
#define RECIRCULATION_RATIO   0.50     /* lean tuning sweep: RR=0.50 */
 ...
 *--------------------------------------------------------------------------*/
static real g_T_flue    = 1327.0;    /* K   - from last converged run      */
static real g_mdot_flue = 6.930e-6;  /* kg/s - 1.3860e-5 x 0.50          */
static real g_Yi_flue[N_SPECIES] = {0.0, 0.77, 0.0, 0.15};
                                      /* nh3, n2, h2, o2 - from iter 120  */
...
DEFINE_ADJUST(flue_recirculation_coupling, domain)
{
    ...
    /* --- Sanity guard --- */
    if (T_out < 300.0 || T_out > 4000.0) return;
